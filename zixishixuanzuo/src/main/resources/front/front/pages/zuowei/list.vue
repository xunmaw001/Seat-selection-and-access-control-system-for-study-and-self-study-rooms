<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.zuoweiName" type="text" placeholder="座位名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in zuoweiTypesList" :key="index"
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
                ...
                class="uni-product"
                :style='{"borderRadius":"50rpx","backgroundColor":"#008761"}' v-for="(product,index) in list" :key="index">
        <view class="uni-product-title"
              :style='{"fontSize":"26rpx","lineHeight":"40rpx","color":"#fff","textAlign":"center"}'>
            {{product.zuoweiName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"80rpx","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.zuoweiPhoto?product.zuoweiPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.zuoweiNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#fff","textAlign":"center"}'>
            <text class="">￥{{product.zuoweiNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('zuowei','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('zuowei','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('zuowei','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 座位类型 字段 字典表查询方法
		  	let zuoweiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'zuowei_types'
		  	});
		  	this.zuoweiTypesList = zuoweiTypes.data.list;
		...
		methods: {
				...
				let params = {
                    ...
                    ,zuoweiDelete: 1
							...
							if (this.categoryName != 0) {
                                params.zuoweiTypes = this.categoryName
                            }

                let res = await this.$api.list(`zuowei`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zuowei', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.zuoweiName) {
                    searchForm['zuoweiName'] = this.searchForm.zuoweiName
                }
				if (this.searchForm.zuoweiName) {
					searchForm['zuoweiName'] = this.searchForm.zuoweiName
				}

                let res = await this.$api.list(`zuowei`, searchForm);
				// 如果是第一页数据置空
