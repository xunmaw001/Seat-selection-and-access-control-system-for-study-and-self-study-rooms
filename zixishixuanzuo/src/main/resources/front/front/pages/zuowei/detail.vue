		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" style="color: red;textAlign:left">
						<text style='font-size:20rpx'>¥</text><text style="font-weight:bold;font-size:50rpx">{{detail.zuoweiNewMoney}}</text><text style="font-size: 20rpx;color: #8799A3;margin-left: 10rpx;">
							<text v-if="detail.zuoweiOldMoney!=null">原价￥<text style="text-decoration:line-through">{{detail.zuoweiOldMoney}}</text></text>
                    	</text>
					</view>
				</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.zuoweiContent"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('zuowei', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.zuoweiPhoto ? this.detail.zuoweiPhoto.split(",") : [];
 			this.zuoweiChange();
			},
			async zuoweiChange() {
                //订座
                let _this = this
                let res = await _this.$api.list('zuoweiOrder', {
                    page: 1,
                    limit: 100,
                    zuoweiId: _this.detail.id,
                    zuoweiOrderDelete: 1,
							...
							let data = [
                                {
                                    ...
                                    zuoweiName: _this.detail.zuoweiName,
                                    buyZuoweiTime: _this.buyZuoweiTime,
                                    zuoweiPhoto: _this.swiperList[0],
                                    zuoweiNewMoney: _this.detail.zuoweiNewMoney,
                                }
							]
							uni.setStorageSync('orderGoods', data);
							_this.$utils.jump('../zuoweiOrder/confirm');
						}
					}
				});
			},
